...
- The repaired material-pH conditional-scout teacher conditions complete terrain hypotheses on legal carrier history and replays the causal prefix. Its frozen admission split supported training, but the restricted gate failed: test regret was **0.01907**, exact action agreement **72.7%**, and useful-dispatch recall **0/3**. The learner selected IDLE in every held-out state, so the freely selected causal/recovery subgates failed and final E2 remains blocked. [Machine-readable gate](CoPh-sense/coph_terrain/results/conditional_scout_material_finite_v2_admission/gate.json).
